 ...
 */
 ...
 int __alloc(struct pcb_t *caller, int vmaid, int rgid, int size, int *alloc_addr)
 {
   ...
   if (!caller || !caller->mm || rgid < 0 || rgid >= PAGING_MAX_SYMTBL_SZ || size <= 0 || !alloc_addr) {
     return -1;
   }
   ...
   */
   *alloc_addr = old_sbrk;
   int remain_start = old_sbrk + inc_sz;
   if (remain_start < cur_vma->vm_end)
   {
     struct vm_rg_struct *newrg = (struct vm_rg_struct *)malloc(sizeof(struct vm_rg_struct));
     if (newrg) {
       newrg->rg_start = remain_start;
 ...
 int liballoc(struct pcb_t *proc, uint32_t size, uint32_t reg_index)
 {
   ...
   if (result < 0)
   {
     printf("[liballoc] ERROR: Allocation failed for PID=%d, Region=%d, Size=%d\n", proc->pid, reg_index, size);
     return -1;
   }
   /* By default using vmaid = 0 */
   #ifdef IODUMP
   printf("===== PHYSICAL MEMORY AFTER ALLOCATION =====\n");
   printf("PID=%d - Region=%d - Virtual Address=%08X - Size=%d byte\n", proc->pid, reg_index, addr, size);
